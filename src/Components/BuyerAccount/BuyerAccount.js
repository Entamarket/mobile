...
export default function BuyerAcount() {
  ...
  const deleteAccountHandler = async () => {
    ...
    try {
      await entamarketApi.delete("/buyer/dashboard/delete-account", header);
      await AsyncStorage.removeItem("entamarketToken");
      await AsyncStorage.removeItem("isUser");
      await AsyncStorage.removeItem("userType");
      dispatch(SliceActions.isUserAuth(false));
      dispatch(SliceActions.setIsType(false));
  ...
  const buyerUpdateEmail = () => {
    navigation.navigate("updateBuyerEmail");
  };
  const buyerResetPassword = () => {
    navigation.navigate("buyerResetPass");
  };

  const navigateToUpdateProfile = () => {
    navigation.navigate("updateBuyerProfile");
  };
  ...
  return (
    <ScrollView style={styles.buyerAccount}>
      <View style={styles.buyerHeader}>
        <FontAwesome name="user-o" style={styles.iconStyle} />
        <Text style={styles.HeaderText}>Buyer Account</Text>
      </View>
      ...
      <View style={styles.mainBox}>
        ...
        />
        <AccountCard
          cardName="Update Buyer Profile"
          cardAction={navigateToUpdateProfile}
        />
        <AccountCard cardName="Update Email" cardAction={buyerUpdateEmail} />
        <AccountCard
          cardName="Reset Password"
          cardAction={buyerResetPassword}
        />

        <AccountCard cardName="Customer Support" cardAction={customerSupport} />

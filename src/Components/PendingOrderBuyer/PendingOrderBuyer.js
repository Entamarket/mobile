...
const PendingOrderBuyer = () => {
  ...
  useEffect(() => {
    const getPendingOrder = async () => {
      ...
      await entamarketApi
        .get(`/delivery/get-single-pending-delivery?checkoutID=${id}`, header)
        .then((resp) => {
          setOrderInfo(resp.data.pendingDelivery.purchases);
          setLoading(false);
        })
        .catch((error) => {
  ...
  return (
    <ScrollView style={styles.container}>
        ...
        return (
          <View key={Math.random() * 10000}>
            <PendingInfo
              ...
              typeName="Seller"
              sellerName={`${item.trader.firstName} ${item.trader.lastName}`}
              sellerContact={item.trader.phoneNumber}
      ...
      <View style={styles.mainBoxBtn}>
        <ButtonPrimary value="Confirm Order" btnAction={showConfirmHandle} />
      </View>
...
export default PendingOrderBuyer;

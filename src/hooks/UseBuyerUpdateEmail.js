...
export default function useSignInHandler() {
  ...
  const buyerUpdateEmailInfo = async (details) => {
    setIsLoading(true);
    ...
    try {
      await entamarketApi
        .put(`/buyer/dashboard/update-email`, details, header)
        .then((resp) => {
          setToken(resp.data.entamarketToken);
          navigation.navigate("buyerVerifyOTP");
          setIsLoading(false);
        });
    } catch (error) {
  ...
  const buyerUpdateEmailHandler = () => {
    if (updateInfo.email === "") {
      ...
      buyerUpdateEmailInfo(updateInfo);
    }
  };

  return [error, setUpdateInfo, buyerUpdateEmailHandler, isLoading, setError];
}
